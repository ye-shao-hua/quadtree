...
int main() {
  {
    ...
    x=0.68+0.6125;
    y=6.9395-0.001;
    quadtree::Quadtree q{quadtree::Rect{-0.6125f, 0.68f, 0.001f, 6.9395f}};
    q.refine_global(4);
    std::clog << "begin\n";
      ...
      for (int i = 0; i < 1000; ++i) {
        ...
        */
        ...
        q.set_value(quadtree::Trapezium{quadtree::Point{-0.6125f,0.002}, x,
                                        0.01f, -1e-10f, 1e-10f},
                    0.f);
        q.set_value(quadtree::Trapezium{quadtree::Point{-0.6125f, 6.9394f}, x,
                                        0.01f, -1e-10f, 1e-10f},
                    0.f);
        q.set_value(quadtree::Trapezium{quadtree::Point{-0.6125f, 0.002f}, 0.01f,
                                        6.935f, -1e-10f, 1e-10f},
                    0.f);
    ...
    for(auto i=0;i<1000;++i){
        for(auto j=0;j<1000;++j){
            std::cout<<q(-0.5+dx*i,0.001+dy*j)<<",";
        }
        std::cout<<"\n";
    }
    std::cout<<x<<" "<<y<<"\n";
